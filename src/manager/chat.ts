...
export class ChatProviderManager {
    ...
    loginWithEmailPassword = async (config: AuthConfig): Promise<LoginResponse> => {
        return this.chatProvider.loginWithEmailPassword(config);
    };
    ...
    sendMessage = (
        ...
        if (!!currentUserInfo) {
            if (!!parentTimestamp) {
                // TODO: implement thread reply
                // return this.chatProvider.sendThreadReply(content, currentUserInfo.id, streamId, parentTimestamp);
            } else {
                return this.chatProvider.sendMessage(channelId, topic, content);
            }
        }
    ...
    async loadChannelHistory(channelId: string, topic: string | undefined): Promise<void> {
        try {
            const messages = await this.chatProvider.loadChannelHistory(channelId, topic);
            return this.updateMessages(channelId, messages);
        } catch (error) {
            Logger.log(`loadChannelHistory error ${error}`);
        }
    }

    updateNewChatState(isNewChat: boolean) {
        this.chatProvider.updateNewChatState(isNewChat);
    }
    ...
    fetchChannels = async (): Promise<Channel[]> => {
        const channels = await this.chatProvider.fetchChannels();
    ...
    fetchStreams = async (): Promise<Channel[]> => {
        const channels = await this.chatProvider.fetchChannels();
        const streams = channels.filter(channel => channel.type === ChannelType.stream);
        return streams;
    };

    fetchTopics = async (streamId: number): Promise<Topic[]> => {
        return this.chatProvider.fetchTopics(streamId);
    };
    ...
    aiCompleteCode(userPrompt: string): Promise<string> {
        if (!this.chatProvider.aiCompleteCode) {
            return Promise.resolve('');
        }
        return this.chatProvider.aiCompleteCode(userPrompt);
    }
}
